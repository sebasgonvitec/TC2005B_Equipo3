...
public class EnemyMovePlay : MonoBehaviour
{
    ...
    bool IsHittingWall()
    {
        ...
        if(Physics2D.Linecast(castPos.position, targetPos, 1 << LayerMask.NameToLayer("Ground")) || Physics2D.Linecast(castPos.position, targetPos, 1 << LayerMask.NameToLayer("Boxes")) || Physics2D.Linecast(castPos.position, targetPos, 1 << LayerMask.NameToLayer("Levers")))
        {
            val = true;
        }
        else
